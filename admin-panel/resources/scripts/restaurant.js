...
function menu() {
	...
	var is_found = false;
	var left_menu_item_no = 0;
	$('#menu').children().each(function(findex, felement) {
		$(felement).children('ul').children().each(function(sindex, selement) {
			if ($(selement).children('a').hasClass('selected')) {
				is_found = true;
				return;
			}
        });
		if (is_found) {
			is_found = false;
			left_menu_item_no = findex;
			return;
		}
    });
	$("#menu").accordion({
		active: left_menu_item_no,
		collapsible: true,
		autoHeight: false
	});
